...
**Estado de implementación (actualizado):** el cliente Kotlin ya tiene una implementación HTTP/WebSocket real de las 11 interfaces (`data/repository/*Remoto.kt`, sobre Ktor), apagada por defecto (`ConfiguracionApi.USAR_BACKEND_REMOTO = false`) hasta que exista un backend que cumpla este documento. Eso significa que las rutas de aquí ya NO son solo una sugerencia en el aire: son las que el cliente de verdad va a golpear. Si el equipo de Go prefiere otras rutas o formas de payload, avisar para actualizar el cliente — el cambio queda contenido a esos 11 archivos, ningún ViewModel ni Vista los conoce. Dos piezas se añadieron ya en el cliente y NO estaban en la version original de este documento: la propuesta de cita desde el médico (sección 9, addendum al final) y los adjuntos del chat (sección 8, addendum al final) — ambas nacieron de requerimientos posteriores a este documento.
...
Sugerencia concreta para Go: un solo endpoint WebSocket por sesión (`WS /realtime?token=jwt`) que multiplexa canales (`chat:{idConversacion}`, `agenda:{idMedico}`) con un envelope tipo:
```json
{ "canal": "agenda:doc_xxx", "evento": "cita_actualizada", "payload": { "...": "Cita" } }
```
es más simple de operar que un socket por contrato, y encaja con que el cliente ya inyecta el reloj/sesión una sola vez.

---

## 12. Addendum — propuesta de cita desde el médico

No estaba en la sección 9 original: nació de un requerimiento posterior ("que el médico pueda agendar, pero el paciente confirma"). Mismo contrato `CitasRepositorio`, mismo `Cita`, un estado nuevo (`PROPUESTA_MEDICO`) y tres métodos nuevos. Rutas **sugeridas por el cliente**, no confirmadas con el backend:

| Método cliente | Endpoint sugerido | Body | Respuesta |
|---|---|---|---|
| `proponerCita(idMedico, idPaciente, idFranja, contacto, ahora)` | `POST /franjas/{idFranja}/propuesta` | `{"idPaciente": "pac_xxx", "contacto": {"...": "DatosContactoCita"}, "ahora": "ISO-8601"}` | `Cita` (nace en `PROPUESTA_MEDICO`) |
| `aceptarPropuesta(idCita)` | `POST /citas/{idCita}/aceptar-propuesta` | — | `Cita` (pasa a `CONFIRMADA`) |
| `rechazarPropuesta(idCita)` | `POST /citas/{idCita}/rechazar-propuesta` | — | `Cita` (pasa a `CANCELADA`, la franja se libera) |

Reglas de negocio, iguales a las de `reservarTemporalmente`/`confirmarCita` pero en sentido inverso — aquí el consultorio agenda y el paciente valida:

- `proponerCita` ocupa la franja de inmediato (como cualquier cita viva); falla con `FRANJA_OCUPADA` en las mismas condiciones que `reservarTemporalmente`.
- `aceptarPropuesta`/`rechazarPropuesta` deben fallar si la cita ya no está en `PROPUESTA_MEDICO` (por ejemplo, si el médico ya la retiró) — el cliente no distingue ese motivo de un `SIN_CONEXION` genérico hoy; si el backend quiere un motivo propio para ese caso, avisar para tipificarlo.
- El médico **nunca** puede confirmar su propia propuesta: no hay endpoint para eso, y no debe agregarse — confirmar lo que uno mismo propuso no valida nada.

---

## 13. Addendum — adjuntos del chat (fotos, archivos, documentos escaneados)

Tampoco estaba en la sección 8 original: nació de un requerimiento posterior ("adjuntar fotos/archivos y escanear documentos en el chat"). El cliente ya sube y descarga adjuntos de verdad (ML Kit Document Scanner en Android, VisionKit en iOS, para el escaneo); esto es lo que le falta al backend.

**Idea central:** los bytes del archivo NUNCA viajan dentro del JSON de un mensaje. Se suben aparte, una vez, y el mensaje solo lleva una referencia.

| Paso | Endpoint sugerido | Detalle |
|---|---|---|
| 1. Subir el archivo | `POST /conversaciones/{idConversacion}/adjuntos` | `multipart/form-data` con dos campos: `tipo` (texto: `FOTO`\|`ARCHIVO`\|`ESCANEO`) y `archivo` (el binario, con su `Content-Type` y `filename` reales) |
| 2. Enviar el mensaje | `POST /conversaciones/{idConversacion}/mensajes` | Igual que la sección 8, más un campo `adjunto` opcional (ver abajo) |

Respuesta del paso 1 (referencia del adjunto ya subido):
```json
{ "idAdjunto": "adj_xxx", "tipo": "FOTO", "nombre": "IMG_20260910.jpg",
  "tipoMime": "image/jpeg", "url": "https://.../archivos/adj_xxx" }
```
`idAdjunto` lo asigna el **backend** en la respuesta de la subida (el cliente genera uno temporal solo para nombrar el archivo en su propio disco antes de subirlo; el que cuenta después es el que devuelve el servidor). `url` es la ruta de descarga que el cliente usará más tarde para bajar el archivo a quien lo reciba — debe ser accesible con el mismo `Authorization: Bearer` de la sesión.

El campo `adjunto` de un mensaje (tanto al enviarlo como al leerlo en `obtenerHistorial`) es ese mismo objeto, o ausente si el mensaje no lleva adjunto:
```json
{ "idMensaje": "msg_xxx", "autor": "PACIENTE", "texto": "", "instante": "ISO-8601",
  "tipo": "NORMAL",
  "adjunto": { "idAdjunto": "adj_xxx", "tipo": "FOTO", "nombre": "...", "tipoMime": "...", "url": "..." } }
```
`texto` puede ir vacío si el mensaje es solo un adjunto (igual que cualquier chat común); nunca los dos vacíos a la vez, eso ya lo impide el cliente antes de golpear la red.

Notas para el backend:
- `GET` sobre la `url` de descarga debe aceptar el mismo Bearer token que el resto de la API — el cliente no tiene un mecanismo de descarga sin sesión.
- No hay endpoint de borrado de adjuntos todavía; si se necesita (por ejemplo, al borrar un mensaje), avisar para agregarlo al contrato.
- El cliente no reintenta la subida si falla a medio mandar el mensaje: un archivo subido sin mensaje asociado es un huérfano aceptado por ahora — si el backend quiere limpiarlos, tendría que ser con una tarea propia (TTL sobre adjuntos sin mensaje), no algo que el cliente le pida.
